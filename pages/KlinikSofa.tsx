...
import { ArrowRight, Armchair, Palette, CheckCircle2, HelpCircle, PenTool, type LucideIcon } from 'lucide-react';
...
// benar-benar dipakai SOFA_SERVICES.
      ...
      <section className="bg-orange-900 text-white py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center gap-8">
          <div className="flex-1">
             ...
             <a href={buildWaHref("Halo Admin, saya mau tanya harga untuk Custom/Service Sofa.")} target="_blank" rel="noreferrer" className="bg-white text-orange-900 font-bold px-8 py-3 rounded-full hover:bg-orange-50 transition-colors inline-block">Konsultasi Sofa</a>
          </div>
          <div className="flex-1 flex justify-end">
             <div className="w-full max-w-md h-64 bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 relative">
                 <div className="absolute -top-4 -right-4 bg-orange-600 text-white p-4 rounded-xl shadow-lg"><Palette size={32} /></div>
                ...
                <ul className="space-y-3">
                   {['Banyak Pilihan Kain', 'Busa High Density', 'Rangka Kayu Oven', 'Jahitan Rapi'].map((item, i) => (
                      <li key={i} className="flex items-center gap-2"><CheckCircle2 className="text-orange-400" size={18} /><span className="text-slate-200 text-sm">{item}</span></li>
                   ))}
                </ul>
             </div>
          </div>
                ...
                <ScrollReveal key={service.id} delay={idx * 50} className="bg-white dark:bg-bg-dark rounded-3xl shadow-lg border border-slate-100 dark:border-slate-800 overflow-hidden">
                   <div className="grid grid-cols-1 md:grid-cols-12 gap-0 md:gap-8">
                      ...
                      <div className="md:col-span-8 p-8 md:py-12 md:pr-12 flex flex-col justify-center">
                         ...
                         <a href={buildWaHref(`Halo Sano Care, saya tertarik dengan layanan ${service.title}`)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-orange-600 dark:text-orange-400 font-bold hover:underline">Info Harga {service.title} <ArrowRight size={20} /></a>
                      </div>
                   </div>
                </ScrollReveal>
             );
